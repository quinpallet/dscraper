...
def main():
    ...
    parser.add_argument('arg_endpoint', help='エンドポイント')
    parser.add_argument('-p', '--params', type=json.loads, help='リクエストパラメータ(JSON形式)')
    parser.add_argument('-m', '--method', default='GET', help='HTTPメソッド(GET/POST): デフォルト値はGET')
    parser.add_argument('-s', '--selector', help='スクレイプ対象のCSSセレクタ')
    parser.add_argument('-ds', '--dynamic_selector', help='遅延ロードされるDOMのCSS親セレクタ')
    args = parser.parse_args()

    scrape = DyScrape(args.arg_endpoint, params=args.params, method=args.method)
    scrape.fetch(selector=args.selector, dselector=args.dynamic_selector)
